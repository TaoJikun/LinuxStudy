...
void *masterstate(void *arg1)//这个函数用来检测发送心跳包的master是否正常
{
    while(flag7==0)
    {
        if(masterstatemap.size()>0)
        {
            ...
            for(auto iter = masterstatemap.begin(); iter != masterstatemap.end(); iter++)
            {
                ...
                if(iterr!=premasterstatemap.end())
                {
                    ...
                    {
                        //LogFile* arg = (LogFile*)arg1;
                        //arg->LogInfo("delete unhealthy cache : ip = %s, port = %s",(char*)masterip.c_str(),(char*)masterport.c_str());
                        cout<<IPportlist.size()<<endl;
                        for(int i=0;i<IPportlist.size();i++)
                            cout<<IPportlist[i]<<endl;
                        for(int i=0;i<IPportlist.size();i++)
                        {
                            ...
                            int flags=fcntl(cfd,F_GETFL,0); //将已连接的套接字设置为非阻塞模式
