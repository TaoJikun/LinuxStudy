...
void* masterhandler(void* arg1)
{
	...
	int size = sizeof(ep) / sizeof(struct epoll_event);
	printf("等待客户端连接...\n");
	for(;;)
	{
		int nready = Epoll_wait(efd, ep, size, -1);
		//等待对端链接
		for(int i=0;i<nready;i++)
		{
			...
			else
			{
				...
				else if(0>n)//接收数据失败了
				{
					...
					if(json::accept(buf))
						info = json::parse(buf);
					if (info["type"]==SPARE_MASTER && flag7==0)
					{
...
{
	LogFile logfile;
    logfile.OpenLogFile("../master2.log");
	...
	//检测主master状态
	pthread_t tid_masterstate;
    Pthread_create(&tid_masterstate, NULL, masterstate, NULL);
	//检测是否所有心跳包都正常
